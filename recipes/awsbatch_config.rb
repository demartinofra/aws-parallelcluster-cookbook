...
if !node['cfncluster']['custom_awsbatchcli_package'].nil? && !node['cfncluster']['custom_awsbatchcli_package'].empty?
  ...
  bash "install aws-parallelcluster-awsbatch-cli" do
    ...
    code <<-CLI
      set -e
      curl --retry 3 -L -o aws-parallelcluster-custom-cli.tgz #{node['cfncluster']['custom_awsbatchcli_package']}
      mkdir aws-parallelcluster-custom-cli
      tar -xzf aws-parallelcluster-custom-cli.tgz --directory aws-parallelcluster-custom-cli
